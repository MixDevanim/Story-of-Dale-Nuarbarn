...
Texture.prototype.loadFile = function(url, onloaded){
	const texture = this;
	...
	image.onload = function() {
		...
		gl.readPixels(0, 0, texture.width, texture.height, gl.RGBA, gl.UNSIGNED_BYTE, texture.data);
		if (onloaded)
			onloaded(texture);
	}
	image.src = url;
...
Texture.prototype.free = function(){
	gl.bindTexture(gl.TEXTURE_2D, this.gltexture);
	gl.deleteTextures(this.gltexture);
}
	...
	for (let i = 0; i < textures.length; i++){
		...
		for (let y0 = 0; y0 < res; y0++){
			for (let x0 = 0; x0 < res; x0++){
				data[((y + y0) * size + (x + x0)) * 4 + 0] = sub.data[(y0*res+x0) * 4 + 0];
				data[((y + y0) * size + (x + x0)) * 4 + 1] = sub.data[(y0*res+x0) * 4 + 1];
				data[((y + y0) * size + (x + x0)) * 4 + 2] = sub.data[(y0*res+x0) * 4 + 2];
				data[((y + y0) * size + (x + x0)) * 4 + 3] = sub.data[(y0*res+x0) * 4 + 3];
			}
		}
	}
